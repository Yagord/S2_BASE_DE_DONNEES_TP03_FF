...
BEGIN
  ...
  WHILE (c_heuresdevol%FOUND) LOOP
    DBMS_OUTPUT.PUT_LINE('L''avion ' || l_numAvion || ' a volé ' || l_heuresdevol || ' heures');
...
BEGIN
  FOR l_heuresdevol IN c_heuresdevol LOOP
    DBMS_OUTPUT.PUT_LINE('L''avion ' || l_heuresdevol.numAvion || ' a volé ' || l_heuresdevol.heuresdevol || ' heures');
...
SET SERVEROUTPUT ON
BEGIN
  FOR l_villeDep IN (SELECT DISTINCT Trajet.villeDep
                      FROM Trajet) LOOP
    ...
    FOR l_heuresdevol IN (SELECT Trajet.numAvion, Avion.nomAvion, Trajet.dateDep, Trajet.VilleArr
                            ...
                            WHERE Trajet.villeDep = l_villeDep.villeDep) LOOP
      
      DBMS_OUTPUT.PUT_LINE('    L''avion ' || l_heuresdevol.numAvion || ' - ' || l_heuresdevol.nomAvion || ' – volera le ' || TO_CHAR(l_heuresdevol.dateDep, 'DD/MM/YYYY')
      || ' à ' || TO_CHAR(l_heuresdevol.dateDep, 'HH24:MI') || ' pour la ville de ' || l_heuresdevol.VilleArr);
    END LOOP;
  END LOOP;
END;
/

SELECT Trajet.VilleDep, Trajet.numAvion, Avion.nomAvion, Trajet.dateDep, Trajet.VilleArr
                          FROM Trajet INNER JOIN Avion ON (Trajet.numAvion = Avion.numAvion)
                          ORDER BY VilleDep;